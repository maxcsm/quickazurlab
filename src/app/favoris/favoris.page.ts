...
this.router.navigateByUrl('/account/' + item.location_id.id);
}

async delete(event: any, item: { favoris_id: number; }) {
     this.id=item.favoris_id;
       ...
       buttons: [
         {
           ...
           text: 'Oui',
           handler: () => { 
       this.redditService.delete("favoris",this.id)  
         .toPromise()
         .then((response) =>{
        setTimeout(() => { 
      this.getData();
